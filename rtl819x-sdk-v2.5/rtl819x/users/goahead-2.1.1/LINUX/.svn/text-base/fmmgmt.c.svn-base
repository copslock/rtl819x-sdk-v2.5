...
///////////////////////////////////////////////////////////////////////////////
#ifdef CONFIG_SNMP
void formSetSNMP(webs_t wp, char_t *path, char_t *query)
{
        ...
        strValue = (char *)websGetVar(wp, T("snmp_enabled"), T(""));
        ...
        if (!apmib_set(MIB_SNMP_ENABLED, (void *)&snmpEnabled)) {
                strcpy(tmpBuf, T("Set SNMP enabled error!"));
                goto setErr;
        }

        strValue = (char *)websGetVar(wp, T("snmp_name"), T(""));
        if (strValue[0]) {
                if (!apmib_set(MIB_SNMP_NAME, (void *)strValue)) {
                        strcpy(tmpBuf, T("Set SNMP location error!"));
                        goto setErr;
                }
        }

        strValue = (char *)websGetVar(wp, T("snmp_location"), T(""));
        if (strValue[0]) {
        ...
        strValue = (char *)websGetVar(wp, T("snmp_contact"), T(""));
        if (strValue[0]) {
                if (!apmib_set(MIB_SNMP_CONTACT, (void *)strValue)) {
                        strcpy(tmpBuf, T("Set SNMP contact error!"));
                        goto setErr;
                }
        }
		
        strValue = (char *)websGetVar(wp, T("snmp_rwcommunity"), T(""));
        if (strValue[0]) {
                if (!apmib_set(MIB_SNMP_RWCOMMUNITY, (void *)strValue)) {
                        strcpy(tmpBuf, T("Set SNMP community error!"));
                        goto setErr;
                }
        }


        strValue = (char *)websGetVar(wp, T("snmp_rocommunity"), T(""));
        if (strValue[0]) {
        ...
        strValue = (char *)websGetVar(wp, T("snmp_trap1"), T(""));
        if (strValue[0]) {
                if (!inet_aton(strValue, &ip) ) {
                        strcpy(tmpBuf, T("Invalid Trap Receiver 1 IP-address value!"));
                        goto setErr;
                }
                if (!apmib_set(MIB_SNMP_TRAP_RECEIVER1, (void *)&ip)) {
                        strcpy(tmpBuf, T("Set Trap Receiver 1 IP-address error!"));
                        goto setErr;
                }
        }

        strValue = (char *)websGetVar(wp, T("snmp_trap2"), T(""));
        if (strValue[0]) {
                if (!inet_aton(strValue, &ip) ) {
                        strcpy(tmpBuf, T("Invalid Trap Receiver 2 IP-address value!"));
                        goto setErr;
                }
                if (!apmib_set(MIB_SNMP_TRAP_RECEIVER2, (void *)&ip)) {
                        strcpy(tmpBuf, T("Set Trap Receiver 2 IP-address error!"));
                        goto setErr;
                }
        }

        strValue = (char *)websGetVar(wp, T("snmp_trap3"), T(""));
        if (strValue[0]) {
                if (!inet_aton(strValue, &ip) ) {
                        strcpy(tmpBuf, T("Invalid Trap Receiver 3 IP-address value!"));
                        goto setErr;
                }
                if (!apmib_set(MIB_SNMP_TRAP_RECEIVER3, (void *)&ip)) {
                        strcpy(tmpBuf, T("Set Trap Receiver 3 IP-address error!"));
                        goto setErr;
                }
        }
...
void formUploadFromUsb(webs_t wp, char_t * path, char_t * query)
{
	int oneReadMax = 4096;
	int oneRead = 0;
	int fileLen=0;
	...
	char lan_ip_buf[30];
	 ...
	 {
      		strcpy(tmpBuf, T("Error!form ware is not exist in usb storage!\n"));
	 	goto ret_err;
	 }
	fd = open(USB_UPLOAD_FORM_PATH, O_RDONLY);
	if (!fd){
      		strcpy(tmpBuf, T("Open image file  failed!\n"));
	 	goto ret_err;
	}
	...
	/* read image from file to buff */
	 do{
		 ...
		 if(buff == NULL)
		 {
      			strcpy(tmpBuf, T("my god breallco failed !\n"));
	 		goto ret_err;
		 }
...
while(head_offset <   wp->lenPostData) {
	...
	#else
				!memcmp(&wp->postData[head_offset], CURRENT_SETTING_HEADER_TAG, TAG_LEN) ||
...
*/
...
#if defined(CONFIG_RTL_92D_SUPPORT)		
	tmpStr = websGetVar(wp, "wlBandMode", T(""));
	if(tmpStr[0] != NULL)
	{
	...
	else if(wlBandMode == BANDMODESINGLE)
	{
		...
		tmpStr = websGetVar(wp, "Band2G5GSupport", T("")); //wlan0 PHYBAND_TYPE
		if(tmpStr[0] != NULL)
		{
...
//displayPostDate(wp->postData);
	...
	else
	{
		...
		tmpStr = websGetVar(wp, "Band2G5GSupport", T(""));
