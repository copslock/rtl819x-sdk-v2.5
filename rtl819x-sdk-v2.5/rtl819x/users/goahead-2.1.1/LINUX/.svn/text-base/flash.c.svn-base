...
unsigned int flash_mib_checksum_ok(int offset);
...
// added by rock /////////////////////////////////////////
	...
	else if(0==strcmp(argv[argNum],"wapi-check"))
	{
		...
		else if ( !strcmp(argv[argNum], "clearppp") ) 	//To send PPP LCP(Termination Request)
		{
			...
			unsigned char * pppServMacPtr=argv[++argNum];
		...
		else if ( !strcmp(argv[argNum], "clearl2tp"))
		{
			...
			unsigned char * lanIp = argv[++argNum];
			unsigned char * serverIp = argv[++argNum];
			unsigned char * l2tp_Buff = argv[++argNum];
...
//mib_display_tlv_content(HW_SETTING, data, header.len);				
...
#if defined(CONFIG_RTL_8196B)	
	strcpy(pMib->deviceName, "RTL8196b");
#elif defined(CONFIG_RTL_8196C) || defined(CONFIG_RTL_8198)
	strcpy(pMib->deviceName, "RTL8196c");
#else
	strcpy(pMib->deviceName, "RTL865x");
#endif

	strcpy(pMib->domainName,"Realtek");
		...
		for (i=0; i<(NUM_VWLAN_INTERFACE+1); i++) {		
			...
			if (idx!=0)
				pMib->wlan[idx][i].wlanDisabled =1;
                       ...
                       {
                       	    if(0 == idx)
                               	sprintf(pMib->wlan[idx][i].ssid, "RTK 11n AP VAP%d", i);
				    else
					sprintf(pMib->wlan[idx][i].ssid, "RTK 11n AP %d VAP%d", idx,i);
                       }				   
                       else if(i == 0)
#endif			
			  {
...
#else
					if(0 == idx)
                               	strcpy(pMib->wlan[idx][i].ssid, "RTK 11n AP" );
				    else
					sprintf(pMib->wlan[idx][i].ssid, "RTK 11n AP %d",idx);
#endif
			  }				   
			...
			{	
				...
				strcpy(pMib->wlan[idx][i].wdsWepKey, "0000000000" );		
	...
	strcpy(pMib->usb3g_idleTime, "300");
	strcpy(pMib->usb3g_connType, "0");
	...
	strcpy(pMib->usb3g_mtuSize,"1490");
...
sprintf(pMib->VlanConfigArray[vlanIdx++].netIface, "%s", "eth0");
sprintf(pMib->VlanConfigArray[vlanIdx++].netIface, "%s", "eth2");
sprintf(pMib->VlanConfigArray[vlanIdx++].netIface, "%s", "eth3");
sprintf(pMib->VlanConfigArray[vlanIdx++].netIface, "%s", "eth4");
sprintf(pMib->VlanConfigArray[vlanIdx++].netIface, "%s", "wlan0");
sprintf(pMib->VlanConfigArray[vlanIdx++].netIface, "%s", "wlan0-va0");
sprintf(pMib->VlanConfigArray[vlanIdx++].netIface, "%s", "wlan0-va1");
sprintf(pMib->VlanConfigArray[vlanIdx++].netIface, "%s", "wlan0-va2");
sprintf(pMib->VlanConfigArray[vlanIdx++].netIface, "%s", "wlan0-va3");
sprintf(pMib->VlanConfigArray[vlanIdx++].netIface, "%s", "wlan1");
sprintf(pMib->VlanConfigArray[vlanIdx++].netIface, "%s", "wlan1-va0");
sprintf(pMib->VlanConfigArray[vlanIdx++].netIface, "%s", "wlan1-va1");
sprintf(pMib->VlanConfigArray[vlanIdx++].netIface, "%s", "wlan1-va2");
sprintf(pMib->VlanConfigArray[vlanIdx++].netIface, "%s", "wlan1-va3");
sprintf(pMib->VlanConfigArray[vlanIdx++].netIface, "%s", "eth1");
#if defined(CONFIG_RTK_VLAN_NEW_FEATURE)
sprintf(pMib->VlanConfigArray[vlanIdx++].netIface, "%s", "eth7");
#endif

#if defined(CONFIG_RTL_8198_AP_ROOT) && defined(GMII_ENABLED)
sprintf(pMib->VlanConfigArray[vlanIdx++].netIface, "%s", "eth6");
#endif
...
//#endif	// CONFIG_RTK_MESH && _MESH_ACL_ENABLE_
	...
	case RADVDPREFIX_T:
		...
		radvdCfgParam.interface.AdvReachableTime=atoi(val[9]);
		radvdCfgParam.interface.AdvRetransTimer=atoi(val[10]);
		...
		radvdCfgParam.interface.prefix[0].AdvValidLifetime=atoi(val[19]);
		radvdCfgParam.interface.prefix[0].AdvPreferredLifetime=atoi(val[20]);
		...
		radvdCfgParam.interface.prefix[1].AdvValidLifetime=atoi(val[28]);
		radvdCfgParam.interface.prefix[1].AdvPreferredLifetime=atoi(val[29]);
...
static int generateHostapdConf(void)
{
	...
	unsigned char line[1024], strValue[1024];
...
static int updateWscConf(char *in, char *out, int genpin, char *wlanif_name)
{
	...
	int is_repeater_enabled=0, isUpnpEnabled=0, wsc_method = 0, wsc_auth=0, wsc_enc=0;
...
//	}
	...
	/*switch to  wlan1 , get mib from wlan1*/
...
{	
	...
	flash_read(&compHeader, offset, sizeof(COMPRESS_MIB_HEADER_T));	
	if((strncmp(compHeader.signature, pcomp_sig, COMP_SIGNATURE_LEN) == 0) &&
	...
	) //check whether compress mib data
	{
		...
		flash_read(compPtr, offset, compLen+sizeof(COMPRESS_MIB_HEADER_T));
...
int flash_mib_compress_write(CONFIG_DATA_T type, char *data, PARAM_HEADER_T *pheader, unsigned char *pchecksum)
{
	...
	if(compPtr != NULL && expPtr!= NULL)
	{
		...
		if ( flash_write(compPtr, dst, compLen+sizeof(COMPRESS_MIB_HEADER_T))==0 )
	...
	for(i=0 ; i<1 ; i++)
	{
		...
		//for(index = 1; index<=entryNum ; index++)
		{
			...
			*((char *)&entry) = (char)index;
			...
			string_to_hex(peerMacStr,peerMacAddr,MAC_ADDR_LEN<<1);
...
static int send_l2tp_msg(unsigned short l2tp_ns, int buf_length, unsigned char * l2tp_buff, unsigned char *lanIp, unsigned char *serverIp)
{
	...
	string_to_hex(lanIp,lanIpAddr,4<<1);

	memset(serverIpAddr,0, sizeof(serverIpAddr));
	string_to_hex(serverIp,serverIpAddr,4<<1);

	memcpy((void *)&lanIpAd, (void* )lanIpAddr, 4);
	memcpy((void *)&serverIpAd, (void* )serverIpAddr, 4);

	string_to_hex(l2tp_buff, l2tp_buff_hex, buf_length<<1);
