...
int getInfo(int eid, webs_t wp, int argc, char_t **argv)
{
    ...
    memset(	tmpStr ,'\0',20);
   	...
   	if ( !strcmp(name, T("name")) ) {
	...
	else if(!strcmp(argv[0],T("wan_access_type_s")))
        {
        ...
        else if(!strcmp(argv[0],T("wan_access_type_e")))
        {
#ifdef CONFIG_RTL_8198_AP_ROOT
                websWrite(wp, T("%s"),"-->");
#else
	...
	*/
	...
	else if ( !strcmp(name, T("ackTimeout"))) {
	...
	else if(!strncmp(name,T("USB3G"),strlen("USB3G")))
	{
		...
		return websWrite(wp, "menu.addItem('QoS', 'qos.asp', '', 'Setup QoS');" );
	...
	else if(!strcmp(name,T("snmp_menu")))
	{
#if defined(CONFIG_SNMP)
		return websWrite(wp, "menu.addItem(\"SNMP\", \"snmp.asp\", \"\", \"SNMP Setup\");");
#else
		return websWrite(wp,"");
#endif
	}
		...
		return websWrite(wp, T("Brian 5BGG"));
#else
 		int isWanPhy_Link=0;
...
//		int i;
...
#if defined(CONFIG_SNMP)
    else if (!strcmp(name, T("snmp_name"))) {
    ...
    else if (!strcmp(name, T("snmp_location"))) {
    ...
    else if (!strcmp(name, T("snmp_contact"))) {
    ...
    else if (!strcmp(name, T("snmp_rwcommunity"))) {
	...
	else if (!strcmp(name, T("snmp_rocommunity"))) {
    ...
    else if (!strcmp(argv[0], T("snmp_trap1"))) {
            if (!apmib_get(MIB_SNMP_TRAP_RECEIVER1, (void *)buffer)) {
                    return -1;
            }
            websWrite(wp, T("%s"), inet_ntoa(*((struct in_addr *)buffer)));
            return 0;
    }
    else if (!strcmp(name, T("snmp_trap2"))) {
    ...
    else if (!strcmp(name, T("snmp_trap3"))) {
...
/*-----end-----*/
	...
	else if(!strcmp(argv[0],T("wapiMenu")))
	{
		...
		websWrite(wp,"wlan_wapi = new MTMenu();");
#if !defined(CONFIG_RTL_8196C)
		websWrite(wp,"wlan_wapi.addItem(\"Certification Install\", \"wlwapiinstallcert.asp\", \"\", \"Install Ceritification\");");
#ifdef CONFIG_RTL_WAPI_LOCAL_AS_SUPPORT
		websWrite(wp,"wlan_wapi.addItem(\"Certification Manage\", \"wlwapiCertManagement.asp\", \"\", \"Manage Ceritification\");");
#endif
#endif
		websWrite(wp,"for(i=0; i < wlan_num ; i++){");
		websWrite(wp,"wlan_name= \"wlan\" +(i+1);");
		websWrite(wp,"if(wlan_num == 1)");
		websWrite(wp,"wlan0_wapi = wlan_wapi ;");
		websWrite(wp,"else{");
		websWrite(wp,"if(1 == wlan_support_92D){");
		websWrite(wp,"if(i==0 && wlan1_phyband != \"\"){");
		websWrite(wp,"wlan_name=wlan_name+\"(\"+wlan1_phyband+\")\";");
		websWrite(wp,"}else if(i==1 && wlan2_phyband != \"\"){");
		websWrite(wp,"wlan_name=wlan_name+\"(\"+wlan2_phyband+\")\";");
		websWrite(wp,"}else{");
		websWrite(wp,"continue;}}");
		websWrite(wp,"if(wlBandMode == 3)");	//3:BANDMODESIGNLE
		websWrite(wp,"wlan_name = \"wlan1\";");
		websWrite(wp,"wlan_wapi.addItem(wlan_name);");
		websWrite(wp,"wlan0_wapi= new MTMenu();}");
		websWrite(wp,"wlan0_wapi.addItem(\"Key Update\", get_form(\"wlwapiRekey.asp\",i), \"\", \"Key update\");");
		websWrite(wp,"if(wlan_num != 1)");
		websWrite(wp,"wlan_wapi.makeLastSubmenu(wlan0_wapi);");
		websWrite(wp,"}");
		websWrite(wp,"menu.makeLastSubmenu(wlan_wapi);");
#endif
		return 0;
	}
...
// 8196c (not include POCKET ROUTER) only support wapi psk
	...
	else if(!strcmp(argv[0], T("wapiCertSel")))
	{
...
#endif
	else if(!strcmp(argv[0],T("wapiLocalAsOption")))
	{
#ifdef CONFIG_RTL_WAPI_LOCAL_AS_SUPPORT
		websWrite(wp,"<option value=\"2\"> Use Cert from Local AS </option>");
#endif
		return 0;
	}
	else if(!strcmp(argv[0],T("wapiLocalAsCertsUploadForm")))
	{
#ifdef CONFIG_RTL_WAPI_LOCAL_AS_SUPPORT
		websWrite(wp,"<form method=\"post\" action=\"/goform/formUploadWapiCert\" enctype=\"multipart/form-data\" name=\"uploadCert\">");
		websWrite(wp,"<table border=\"0\" cellspacing=\"0\" width=\"500\">");
		websWrite(wp,"<tr><font size=2></tr>");
		websWrite(wp,"<tr><hr size=1 noshade align=top></tr>");
		websWrite(wp,"<tr><td width=\"0.55\"><font size=2><b>Certificate Type of Local AS:</b></td>");
		websWrite(wp,"<td width=\"0.45\"><font size=2> <input name=\"cert_type\" type=radio value=0 checked>X.509</td></tr>");
		websWrite(wp,"<tr><td width=\"0.55\"><font size=2><b>ASU Certificate from Local AS:</b></td>");
		websWrite(wp,"<td width=\"0.45\"><font size=2><input type=\"file\" name=\"asu_binary\" size=20></td></tr></table>");
		websWrite(wp,"<input onclick=sendClicked(this.form) type=button value=\"Upload\" name=\"send\">&nbsp;&nbsp;");
		websWrite(wp,"<input type=\"reset\" value=\"Reset\" name=\"reset\">");
		websWrite(wp,"<input type=\"hidden\" value=\"/wlwapiinstallcert.asp\" name=\"submit-url\">");
		websWrite(wp,"<input type=\"hidden\" value=\"asu\" name=\"uploadcerttype\"></form>");

		websWrite(wp,"<form method=\"post\" action=\"/goform/formUploadWapiCert\" enctype=\"multipart/form-data\" name=\"uploadUserCert\">");
		websWrite(wp,"<table border=\"0\" cellspacing=\"0\" width=\"500\">");
		websWrite(wp,"<tr><font size=2></tr>");
//		websWrite(wp,"<tr><hr size=1 noshade align=top></tr>");
		websWrite(wp,"<tr><td width=\"0.55\"><font size=2><b>User Certificate from Local AS:</b></td>");
		websWrite(wp,"<td width=\"0.45\"><font size=2><input type=\"file\" name=\"user_binary\" size=20></td></tr></table>");
		websWrite(wp,"<input onclick=sendClicked(this.form) type=button value=\"Upload\" name=\"send\">&nbsp;&nbsp;");
		websWrite(wp,"<input type=\"reset\" value=\"Reset\" name=\"reset\">");
		websWrite(wp,"<input type=\"hidden\" value=\"/wlwapiinstallcert.asp\" name=\"submit-url\">");
		websWrite(wp,"<input type=\"hidden\" value=\"user\" name=\"uploadcerttype\"></form>");
#endif
		return 0;
	}
	...
	else if(!strcmp(argv[0],T("info_country")))
	{
		...
		else
		{
			...
			for(i=0;i<sizeof(countryIEArray)/sizeof(COUNTRY_IE_ELEMENT);i++)
			{
				...
				websWrite(wp,T("[%d,'%s',%d,%d,'%s'"),countryIEArray[i].countryNumber,countryIEArray[i].countryA2,
	...
	else if(!strcmp(argv[0],T("info_2g")))
	{
		...
		else
		{
			...
			{
				websWrite(wp,T("[%d,%d,'%s'"),Bandtable_2dot4G[i].region,Bandtable_2dot4G[i].channel_set,Bandtable_2dot4G[i].area);
	...
	else if(!strcmp(argv[0],T("info_5g")))
	{
		...
		else
		{
			...
			{
				websWrite(wp,T("[%d,%d,'%s'"),Bandtable_5G[i].region,Bandtable_5G[i].channel_set,Bandtable_5G[i].area);
	...
	else if(!strcmp(argv[0],T("country_str")))
	{		
...
int getIndex(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	else if ( !strcmp(name, T("isPureAP")) ) {
#if defined(CONFIG_RTL_8198_AP_ROOT)
                sprintf(buffer, "%d", 1);
#else
                sprintf(buffer, "%d", 0);
...
//for 11N
	...
	else if ( !strcmp(name, T("tx_beamforming"))) {
	...
	else if ( !strcmp(name,T("vlan_bridge_feature"))){	
	#if defined(CONFIG_RTK_VLAN_NEW_FEATURE)
		//if(!apmib_get(MIB_VLANCONFIG_FORWARDING_RULE,(void*)&val))
		sprintf(buffer,"%d",1);	
	#else
		sprintf(buffer,"%d",0);
	#endif
...
#if defined(CONFIG_SNMP)
    else if (!strcmp(name, T("snmp_enabled"))) {
...
//### add by ls 2011.3.30 TX Beamforming added to mib in 92D decide if 92D interface
	else if(!strcmp(name,T("wlan_interface_92D")))
	{
...
//### end
	else if(!strcmp(name,T("wlan_support_92D_concurrent")))
	{
...
//### end
    ...
    else if(!strcmp(argv[0], T("dsf_enable")))
    {
