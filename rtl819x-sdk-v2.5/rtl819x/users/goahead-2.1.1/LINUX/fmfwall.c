...
int vlanList(int eid, webs_t wp, int argc, char_t **argv)
{
...
#if defined(CONFIG_RTK_VLAN_NEW_FEATURE) 
		sprintf(bufStr, "%d|%s|%d|%d|%d|%d|%d|%d|%s|%d", entry.enabled,IfaceName,entry.tagged,0,entry.priority,entry.cfi,0,entry.vlanId,wanLan, forwarding_rule);
#else
	...
	{
#if defined(CONFIG_RTK_VLAN_NEW_FEATURE) 
				sprintf(bufStr, "0|none|0|0|0|0|0|0|LAN|0");
#else
		...
		for(i=1; i<=MAX_IFACE_VLAN_CONFIG ; i++)
		{
			...
			*((char *)&entry) = (char)i;
			...
			if(strTmp[0])
			{
				...
				printf("--%s(%d)--name is %s\n", __FUNCTION__, __LINE__, entry.netIface);
			}
			else
			{
...
void formVlan(webs_t wp, char_t *path, char_t *query)
{
		...
		//	printf("--%s(%d)--i is %d\n", __FUNCTION__, __LINE__, i);
			...
			*((char *)&entry) = (char)i;
			...
			if(strTmp[0])
			{
				...
				if(mapping)
				{
				printf("--%s(%d)--name is %s\n", __FUNCTION__, __LINE__, mapping->ifname);
					strcpy(entry.netIface,mapping->ifname);
				}
				printf("--%s(%d)--name is %s\n", __FUNCTION__, __LINE__, entry.netIface);
			}
	...
	//		printf("i is %d, entry.enabled is %d, entry.tagged is %d, entry.cfi is %d, entry.vlanId is %d, entry.priority is %d\n", i, entry.enabled, entry.tagged, entry.cfi, entry.vlanId, entry.priority);
#if defined(CONFIG_RTK_VLAN_NEW_FEATURE)
				memset(tmpBuf,0x00, sizeof(tmpBuf));
				sprintf(tmpBuf,"vlan_forward_%d",i);
				strTmp = websGetVar(wp, T(tmpBuf), T(""));
...
int ipQosList(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	nBytesSent += websWrite(wp, T("<tr>"
      	"<td align=center width=\"30%%\" bgcolor=\"#808080\"><font size=\"2\"><b>Local IP Address</b></font></td>\n"
      	"<td align=center width=\"25%%\" bgcolor=\"#808080\"><font size=\"2\"><b>MAC Address</b></font></td>\n"
      	"<td align=center width=\"25%%\" bgcolor=\"#808080\"><font size=\"2\"><b>Mode</b></font></td>\n"
      	"<td align=center width=\"25%%\" bgcolor=\"#808080\"><font size=\"2\"><b>Uplink Bandwidth</b></font></td>\n"
      	"<td align=center width=\"25%%\" bgcolor=\"#808080\"><font size=\"2\"><b>Downlink Bandwidth</b></font></td>\n"
	"<td align=center width=\"20%%\" bgcolor=\"#808080\"><font size=\"2\"><b>Comment</b></font></td>\n"
      	"<td align=center width=\"15%%\" bgcolor=\"#808080\"><font size=\"2\"><b>Select</b></font></td></tr>\n"));
	...
	if (strAdd[0]) {
		...
		if(entry.mode &  QOS_RESTRICT_MIN){
			...
			if(ret==QOS_BW_CHECK_FAIL){
				strcpy(tmpBuf, T("checkQosRuleBw fail!"));
				goto setErr;
			}
			else if(ret==QOS_BOTHLINK_BW_OVERSIZE){
				strcpy(tmpBuf, T("Error: for guaranteed minimum bandwidth of both uplink and downlink, the sum bandwidth of all qos rules are larger than the total bandwidth!"));
				goto setErr;
			}
			else if(ret==QOS_DOWNLINK_BW_OVERSIZE){
				strcpy(tmpBuf, T("Error: for guaranteed minimum bandwidth of downlink, the sum bandwidth of all qos rules is larger than the total downlink bandwidth!"));
				goto setErr;
			}
			else if(ret==QOS_UPLINK_BW_OVERSIZE){
				strcpy(tmpBuf, T("Error: for guaranteed minimum bandwidth of uplink, the sum bandwidth of all qos rules is larger than the total uplink bandwidth!"));
				goto setErr;
			}
		}
