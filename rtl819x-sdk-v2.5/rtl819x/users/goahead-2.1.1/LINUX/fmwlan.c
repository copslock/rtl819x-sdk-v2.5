...
//displayPostDate(wp->postData);
	...
	if(strVal[0]){
		...
		if(val != atoi(strVal)){
			...
			if ( apmib_set(MIB_HW_REG_DOMAIN, (void *)&val) == 0) {
					strcpy(tmpBuf, T("Set wlan regdomain error!"));
					goto setErr_wlan;
			}
	...
	if(strVal[0]){
			if (apmib_set(MIB_WLAN_COUNTRY_STRING, (void *)strVal) == 0) {
					strcpy(tmpBuf, T("Set wlan countrystr error!"));
					goto setErr_wlan;
			}
	}
...
int wepHandler(webs_t wp, char *tmpBuf, int wlan_id)
{
	...
	if  (wepKey[0]) {
		if (strlen(wepKey) != keyLen) {
			sprintf(tmpBuf, T("Invalid key length! Expect length is %d"), keyLen);
			goto setErr_wep;
		}
		if ( !isAllStar(wepKey) ) {
...
//					printf("%s(%d): WPA-RADIUS can be used when device is set to client mode\n",__FUNCTION__,__LINE__);//Added for test 
	...
	wapi_end:
		...
		{ 
			...
			strVal = websGetVar(wp, varName, T(""));
			if (!strVal[0]) {
				strcpy(tmpBuf, T("No WAPI cert selected!"));
				goto setErr_encrypt;
			}
			intVal=atoi(strVal);
			...
			for(i=0;i<NUM_VWLAN_INTERFACE+1;i++)
			{
				...
				if ( !apmib_set(MIB_WLAN_WAPI_CERT_SEL, (void *)&intVal)) {
					strcpy(tmpBuf, T("Set WAPI cert sel error!"));
					goto setErr_encrypt;
				}	
			}
		...
		if (wlan_mode == CLIENT_MODE) { // wlan client mode
			...
			if(wlan_idx==wlanIdx_5G){
				...
				if ( !apmib_set(MIB_WLAN_RS_BAND_SEL, (void *)&rsBandSel)) {
					strcpy(tmpBuf, T("Set MIB_WLAN_RS_BAND_SEL error!"));
					goto setErr_encrypt;
				}
			}
			...
			if(intVal == EAP_MD5){
				...
				if(rsBandSel == PHYBAND_5G){
					if(isFileExist(RS_USER_CERT_5G) != 1){
						strcpy(tmpBuf, T("No 802.1x RADIUS 5g user cert!\nPlease upload it."));
						goto setErr_encrypt;
					}
					
					if(isFileExist(RS_ROOT_CERT_5G) != 1){
						strcpy(tmpBuf, T("No 802.1x RADIUS 5g root cert!\nPlease upload it."));
						goto setErr_encrypt;
					}
				}
				else if(rsBandSel == PHYBAND_2G){
					if(isFileExist(RS_USER_CERT_2G) != 1){
						strcpy(tmpBuf, T("No 802.1x RADIUS 2g user cert!\nPlease upload it."));
						goto setErr_encrypt;
					}
					
					if(isFileExist(RS_ROOT_CERT_2G) != 1){
						strcpy(tmpBuf, T("No 802.1x RADIUS 2g root cert!\nPlease upload it."));
						goto setErr_encrypt;
					}
				}
			}
...
{
	char_t *strAuth, *strFragTh, *strRtsTh, *strBeacon, *strPreamble, *strAckTimeout;
	...
	strAckTimeout = websGetVar(wp, T("ackTimeout"), T(""));
	if (strAckTimeout[0]) {
		if ( !string_to_dec(strAckTimeout, &val) || val<0 || val>255) {
			strcpy(tmpBuf, T("Error! Invalid value of Ack timeout."));
			goto setErr_advance;
		}
		if ( apmib_set(MIB_WLAN_ACK_TIMEOUT, (void *)&val) == 0) {
			strcpy(tmpBuf, T("Set Ack timeout failed!"));
			goto setErr_advance;
		}
	}
#if 0
...
// for 11N
	...
	strValue = websGetVar(wp, T("coexist_"), T(""));
		...
		strValue = websGetVar(wp, T("beamforming_"), T(""));
		if (strValue[0]) {
			if (!strcmp(strValue, T("enable")))
				val = 1;
			else
...
void formUploadWapiCert(webs_t wp, char_t * path, char_t * query)	// Only for Local AS
{
...
void formUploadWapiCertAS0(webs_t wp, char_t * path, char_t * query)	// Only for Remote AS0
{
	/*save asu and user cert*/
	char *submitUrl,*strVal;
	char tmpBuf[100];
...
void formUploadWapiCertAS1(webs_t wp, char_t * path, char_t * query)	// Only for Remote AS1
{
	/*save asu and user cert*/
	char *submitUrl,*strVal;
	char tmpBuf[100];
...
void formUpload8021xUserCert(webs_t wp, char_t * path, char_t * query)
{
	...
	if((rsBandSel != PHYBAND_5G) && (rsBandSel != PHYBAND_2G)){
		strcpy(tmpBuf, T("Wrong rsBandSel !"));
		goto upload_ERR;
	}
		...
		if(!strcmp(strVal,"user"))
		{
			...
			else
			{
				...
				if(rsBandSel == PHYBAND_5G){
					...
					if(NULL == fp)
					{
						sprintf(tmpBuf,"[2] Can not open tmp user cert(%s)!", RS_USER_CERT_5G);
						goto upload_ERR;
					}
				}
				else{
					...
					if(NULL == fp)
					{
						sprintf(tmpBuf,"[2] Can not open tmp user cert(%s)!", RS_USER_CERT_2G);
						goto upload_ERR;
					}
				}
		...
		else if(!strcmp(strVal,"root"))
		{
			...
			else
			{
				...
				if(rsBandSel == PHYBAND_5G){
					...
					if(NULL == fp)
					{
						sprintf(tmpBuf,"[2] Can not open tmp RS cert(%s)!", RS_ROOT_CERT_5G);
						goto upload_ERR;
					}
				}
				else{
					...
					if(NULL == fp)
					{
						sprintf(tmpBuf,"[2] Can not open tmp RS cert(%s)!", RS_ROOT_CERT_2G);
						goto upload_ERR;
					}
				}
