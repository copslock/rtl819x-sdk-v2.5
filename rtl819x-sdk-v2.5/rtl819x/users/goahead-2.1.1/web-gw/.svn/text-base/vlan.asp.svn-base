...
<script>
	...
	var vlan_nat_disableTag = 0;
	var vlan_bridge_feature=<% write(getIndex("vlan_bridge_feature"));%>;
	var vlan_forwording_rule_show = 0;

	if(vlan_nat_disableTag==1||vlan_bridge_feature==1)
		vlan_forwording_rule_show=1;
	...
	function init()
	{
		...
		for(i=1;i<=MAXNUM_VLAN;i++)
		{
			...
			if(vlan_forwording_rule_show==1)
	...
	function vlan_table_disabled(index, value)
	{
		...
		if(value == 1)
		{
			if(vlan_nat_disableTag==0 || get_by_id("vlan_forward_sel"+index).value == 1)
			...
			if(get_by_id("wanlan_"+index).value == "WAN" && vlan_forwording_rule_show==1)
			{
				get_by_id("vlan_forward_sel"+index).disabled = true;
				//if(vlan_bridge_feature == 1)
				{					
					get_by_id("tag_"+index).disabled = false;
				}
				/*else
	...
	{
			/*if(vlan_nat_disableTag==1)
			{
				get_by_id("vlan_tag_"+index).value = 0;
				get_by_id("tag_"+index).checked = false;
				if(get_by_id("vlan_forward_sel"+index).value == 1)//bridge mode
					get_by_id("tag_"+index).disabled = false;
				else
					get_by_id("tag_"+index).disabled = true;
			}else*/
	...
	function vlan_forward_select(index, value)
	{
		...
		if(vlan_nat_disableTag==1)
		{
			if(get_by_id("vlan_forward_sel"+index).value == 1)//birdge mode
			{
				get_by_id("tag_"+index).disabled = false;
			}
			else
	...
	function webVlanList(num)
	{
		...
		for (var i = 1; i <= num; i++)
		{
			...
			if(vlan_bridge_feature==1 && DataArray[1]=="Local Host/WAN")
				continue;
