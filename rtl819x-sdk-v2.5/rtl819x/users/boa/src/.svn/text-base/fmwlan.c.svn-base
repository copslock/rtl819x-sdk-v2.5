...
{
  char *strSSID, *strChan, *strDisabled, *strVal, strtmp[20];
	int chan, disabled ;
	NETWORK_TYPE_T net;
	char *strRate;
	int val;
...
void formUploadWapiCert(request *wp, char * path, char * query)	// Only for Local AS
{
	...
	submitUrl = req_get_cstream_var(wp, ("submit-url"), "");   // hidden page
//	printf("submitUrl %s\n",submitUrl);
	strVal = req_get_cstream_var(wp, ("uploadcerttype"), "");
//	printf("uploadcerttype %s\n",strVal);

	if(NULL == strstr(wp->post_data,CERT_START) || NULL ==strstr(wp->post_data,CERT_END))
	{
	...
	{
		fwrite(wp->post_data,wp->post_data_len,0x1,fp);
		fclose(fp);
		strcpy(cmd,"cp ");
		strcat(cmd,TMP_CERT);
		strcat(cmd," ");
		...
		{		
			...
			else
			{
				...
				if(rsBandSel == PHYBAND_5G){
					...
					if(NULL == fp)
					{
						sprintf(tmpBuf,"[2] Can not open tmp user cert(%s)!\nMaybe you should upload your user certificate once again!\n", RS_USER_CERT_5G);
						goto upload_ERR;
					}
				}
				else{
