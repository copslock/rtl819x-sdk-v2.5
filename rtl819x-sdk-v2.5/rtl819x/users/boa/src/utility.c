...
#if defined(CONFIG_RTL_8196C) || defined(CONFIG_RTL_8198)//only support in 8196c or RTL8198
...
while(head_offset <   upload_len) {
    ...
    if(len > 0x400000){ //len check by sc_yang
      		sprintf(buffer, "Image len exceed max size 0x400000 ! len=0x%x</b><br>", len);
		goto ret_upload;
    }
    if ( (flag == 1) || (flag == 3)) {
