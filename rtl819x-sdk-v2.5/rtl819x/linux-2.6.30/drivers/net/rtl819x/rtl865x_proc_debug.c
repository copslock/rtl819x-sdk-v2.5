...
static int32 mac_config_read( char *page, char **start, off_t off, int count, int *eof, void *data )
{
	...
	len += sprintf(page+len, "48 Pass 1 drop: ");
	if (data0)
...
int32 rtl865x_proc_debug_init(void)
{
	...
	if(rtl865x_proc_dir)
	{
		...
		/*qos rules*/
		{
			...
			{
				rtlglue_printf("can't create proc entry for soft_qosRules");
				retval = FAILED;
				goto out;
			}
		}
		...
		/*nic mbuf*/
		{
			...
			{
				rtlglue_printf("can't create nic_mbuf entry for nic_mbuf");
				retval = FAILED;
				goto out;
			}
		}
		...
		/*rx ring*/
		{
			...
			{
				rtlglue_printf("can't create rxRing entry for rxRing");
				retval = FAILED;
				goto out;
			}
		}
		...
		/*tx ring*/
		{
			...
			{
				rtlglue_printf("can't create txRing entry for txRing");
				retval = FAILED;
				goto out;
			}
		}
		...
		/*FC threshold*/
		{
			...
			{
				rtlglue_printf("can't create proc entry for FC threshold");
				retval = FAILED;
				goto out;
			}
		}
