...
static int get_chnl_lmt_dot_new(struct rtl8192cd_priv *priv, unsigned char *line_head, unsigned int *channel, unsigned int *limit, unsigned int *table_idx)
{
	...
	*channel = '\0';
	...
	if (next == NULL)
		...
		next2 = get_digit_dot_8812(&next);
	
	if(next2 == NULL)
		return 0;
	else
		next3 = get_digit_dot_8812(&next2);
	...
	*channel = _atoi(ch, base);
	...
	if(is_NA(next))
		...
		lmt[0] = _convert_2_pwr_dot(next, base);

	if(is_NA(next2))
		lmt[1] = 0;
	else
		lmt[1] = _convert_2_pwr_dot(next2, base);

	if(is_NA(next3))
		lmt[2] = 0;
	else
		lmt[2] = _convert_2_pwr_dot(next3, base);
...
#ifdef TXPWR_LMT_88E //CONFIG_RTL_88E_SUPPORT
	if(GET_CHIP_VER(priv) == VERSION_8188E)
...
int PHY_ConfigBBWithParaFile(struct rtl8192cd_priv *priv, int reg_file)
{
	...
	int 			   file_format=TWO_COLUMN;
	unsigned char      *mem_ptr, *line_head, *next_head=NULL;
#if defined(CONFIG_RTL_88E_SUPPORT) // TXPWR_LMT_88E
...
#if defined(TXPWR_LMT_88E) // TXPWR_LMT_88E
		if(reg_file == PHYREG_PG)
		if(GET_CHIP_VER(priv) == VERSION_8188E)
		{
...
#if defined(TXPWR_LMT_88E) // TXPWR_LMT_88E
				if((GET_CHIP_VER(priv) == VERSION_8188E) && (reg_file == PHYREG_PG))
...
#endif        
        {  // non AC chip & 1T1R
            PHY_SetBBReg(priv, 0xd08, 0xffffffff, 0x00009077);
        }
    }
	DBFEXIT;
