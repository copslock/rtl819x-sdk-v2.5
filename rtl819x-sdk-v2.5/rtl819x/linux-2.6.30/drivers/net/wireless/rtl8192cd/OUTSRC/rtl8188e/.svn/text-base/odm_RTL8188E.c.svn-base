...
#if defined(CONFIG_HW_SMART_ANTENNA)
VOID
odm_RX_HWAntDivInit(
	IN		PDM_ODM_T		pDM_Odm
)
...
)
{
	...
	else if(AntCombination == 7)
	{
		...
		else //MPchip
		{
			...
			ODM_SetBBReg(pDM_Odm, 0x918 , bMaskByte3, 255);
		}
	}
	...
	//Debug training interval
	//ODM_SetBBReg(pDM_Odm, 0xc58, 0xFFFF, 256);
...
///*	
	...
	if(txcfg->pstat){
		#if 0
		pdesc->Dword2 |= set_desc(0<<24);
		pdesc->Dword2 |= set_desc(0<<25);
		pdesc->Dword7 |= set_desc(0<<29);
		#else
...
#if (DM_ODM_SUPPORT_TYPE & (ODM_AP|ODM_ADSL))
				ODM_RT_TRACE(pDM_Odm,ODM_COMP_ANT_DIV, ODM_DBG_LOUD, ("TrainIdx=[%d], MAC Addr = %x:%x:%x:%x:%x:%x\n",pDM_FatTable->TrainIdx,
					pEntry->hwaddr[5],pEntry->hwaddr[4],pEntry->hwaddr[3],pEntry->hwaddr[2],pEntry->hwaddr[1],pEntry->hwaddr[0]));
#else
				ODM_RT_TRACE(pDM_Odm,ODM_COMP_ANT_DIV, ODM_DBG_LOUD, ("TrainIdx=[%d], MAC Addr = %x:%x:%x:%x:%x:%x\n",pDM_FatTable->TrainIdx,
					pEntry->MacAddr[5],pEntry->MacAddr[4],pEntry->MacAddr[3],pEntry->MacAddr[2],pEntry->MacAddr[1],pEntry->MacAddr[0]));
...
)
{
	...
	if(pDM_FatTable->FAT_State == FAT_TRAINING_STATE)
	{
		...
		for (i=0; i<7; i++)
		{
			...
			if(pDM_FatTable->antAveRSSI[i] > MaxRSSI)
			{
				...
				Pckcnt = pDM_FatTable ->antRSSIcnt[i];
				TargetAnt = (u1Byte) i;
			}
			if(pDM_FatTable->antAveRSSI[i] == MaxRSSI)
			{
				if(pDM_FatTable ->antRSSIcnt > Pckcnt)
					{
					MaxRSSI = pDM_FatTable->antAveRSSI[i];
					Pckcnt = pDM_FatTable ->antRSSIcnt[i];
				TargetAnt = (u1Byte) i;
			...
			//ODM_RT_TRACE(pDM_Odm,ODM_COMP_ANT_DIV, ODM_DBG_LOUD, ("FAT_Ant[%d], AvgRSSI = %d, Cnt = %d\n",
			//	i, pDM_FatTable->antAveRSSI[i], pDM_FatTable->antRSSIcnt[i]));
		}
	...
	if(pDM_FatTable->FAT_State == FAT_NORMAL_STATE)
	{
		...
		u1Byte Interval;
		//Debug training interval
		Interval = ODM_GetBBReg(pDM_Odm, 0x918, bMaskByte3);
		ODM_SetTimer(pDM_Odm,&pDM_Odm->FastAntTrainingTimer, Interval); //ms
		}
...
)
{
	...
	*/
	if(pDM_Odm->fat_enable == 0)
		return;

	if(!pDM_Odm->bLinked)
	{
		ODM_RT_TRACE(pDM_Odm,ODM_COMP_ANT_DIV, ODM_DBG_LOUD, ("ODM_FAT_88E(): No Link.\n"));
	...
	else
	{
		...
		{
			ODM_RT_TRACE(pDM_Odm,ODM_COMP_ANT_DIV, ODM_DBG_LOUD, ("---Turn on FAT---\n"));
